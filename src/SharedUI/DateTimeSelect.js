...
class DateTimeSelect extends React.Component {
  ...
  validTimeConstraints = () => {
    ...
    if (typeof currentDate === 'number')
      this.setState({ currentDate: moment(this.state.currentDate) });
  ...
  render() {
    ...
    const validDates = current =>
      current.isAfter(this.props.notToday === undefined ? yesterday : moment());
        ...
        <DateTime
          ...
          inputProps={{
            value: this.state.toggled
              ? moment(this.state.currentDate).format('MMMM Do YYYY, h:mm a')
