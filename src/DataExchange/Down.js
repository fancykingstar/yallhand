...
export const users_and_teams = async (accountID, userID) => {
  ...
  const users = await apiCall_noBody(
    `users/all?filter={"where":{"accountID":"${accountID}"}}`,
    'GET',
  );
  const inactiveUsers = await apiCall_noBody(
    `validations?filter={"where":{"userId":"","accountID":"${accountID}"}}`,
    'GET',
  );
  ...
  const tags = await apiCall_noBody(
    `tags/all?filter={"where":{"accountID":"${accountID}"}}`,
    'GET',
  );
  ...
  if (!UserStore.user.isAdmin) {
    ...
    UserStore.setPreviewTagPath(
      TeamStore.previewValidPath(
        UserStore.user.tags.length === 0 ? 'none' : UserStore.user.tags[0],
        'tag',
      ),
    );
  }
...
export const users = async accountID => {
  const userData = await apiCall_noBody(
    `users/all?filter={"where":{"accountID":"${accountID}"}}`,
    'GET',
  );
  const inactiveUsers = await apiCall_noBody(
  ...
  AccountStore.loadUsers([...userData, ...inactiveUsers]);
  return [...userData, ...inactiveUsers];
};
...
export const allContent = async accountID =>
  apiCall_noBody(`policies/${  accountID}`, 'GET')
    .then(result =>
      PoliciesStore.loadPolicies(
        contentFilter()
          ? validContent(result, UserStore.previewTeamPath, UserStore.previewTagPath)
          : result,
      ),
    )
    .then(() =>
      apiCall_noBody(
        `announcements/all?filter={"where":{"accountID":"${accountID}"}}`,
        'GET',
      ).then(result =>
    ...
    .then(
      () =>
        apiCall_noBody(`fileresources/${  accountID}`, 'GET').then(result =>
          ResourcesStore.loadFiles(
            contentFilter()
              ? validResources(result, UserStore.previewTeamPath, UserStore.previewTagPath)
              : result,
          ),
        ),
    );
...
export const tags = async accountID => {
  const result = await apiCall_noBody(
    `tags/all?filter={"where":{"accountID":"${accountID}"}}`,
    'GET',
  );
...
export const urls = async accountID =>
  apiCall_noBody(`urls/${  accountID}`, 'GET').then(result =>
...
export const logs = async (accountID, userID) => {
  const result = contentFilter()
    ? await apiCall_noBody(`itslogs/views/uniquebyuser/${  userID}`, 'GET')
    : await apiCall('itslogs/views/analyticsdata', 'POST', { accountID }).then(res =>
        res.json(),
      );
  if (contentFilter()) {
...
export const scheduled = async accountID => {
  const result = await apiCall_noBody(
    `schedules/all?filter={"where":{"accountID":"${accountID}","executed":false}}`,
    'GET',
  );
...
export const surveys = async (accountID, userID) => {
  const result = await apiCall(
    'surveys/find',
    'POST',
    !contentFilter() ? { accountID } : { accountID, userID },
  ).then(r => r.json());
  ...
  await PollStore.loadPolls(result.surveys.filter(i => i.type === 'poll'));
};
