...
class Store {
  ...
  sortStructure(path, type) {
    ...
    depth0s.forEach(depth0 => {
      sortedStructure.push({'key': depth0.label, 'value': depth0[id], 'text': depth0.label})
      const depth1s = sortedPath.filter(item => item.parent === depth0[id])
        depth1s.forEach(depth1 => {
          ...
          depth2s = depth2s.map(item => ({'key': item.label, 'value':item[id], 'text':  "-- " + item.label}))
